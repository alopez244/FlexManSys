...
//      if (state!=null) cmd+=" state="+state;
	...
	/**
	 * Devuelve el componente a partir la instancia/implementaciï¿½n. 
	 * 
	 * @param id id de la instancia/implementaciï¿½n
	 * @return
...
//		//esto no se debería hacer siempre
...
//		}
		...
		// el conversationId lo utiliza el MWM para despertar la tarea que interveniene en esta conversaciï¿½n (el id conincide con el id de la tarea)
	...
	public ACLMessage receiveMessage() throws Exception {
		...
		while (aMsg == null){
			//beh.block(); /********* ESTE BLOCK NO ESTï¿½ BIEN UTILIZADO !!! ***/
			try { Thread.sleep(10); } catch (InterruptedException e) { e.printStackTrace(); }
			aMsg = receive(mt);
...
//		}
	...
	public void setState(String cmpIns, String state) throws Exception {
	  ...
	  try { resolveMWM(); } catch (Exception e) {e.printStackTrace();}
		...
		// si el estado es running, ademï¿½s actualizo el estado en 
		// de la instancia a la implementaciï¿½n y de implementaciï¿½n a componente
