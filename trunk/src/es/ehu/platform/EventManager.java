...
public class EventManager extends Agent{

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	static final Logger LOGGER = LogManager.getLogger(EventManager.class.getName()) ;
	/**
	 * 
	 */
	...
	protected void setup() {
		...
		for (int i=0; i<args.length; i++) {
			LOGGER.info(args[i]);
			if (args[i].toString().startsWith("eventid=")) 
				this.eventID = args[i].substring("eventid=".length());
	...
	class EventManagerBehaviour extends SimpleBehaviour 
    {
		...
		public EventManagerBehaviour(Agent agent) {
            super(agent);
        }
