...
import es.ehu.domain.eHealth.functionality.MessageDoctor;
import es.ehu.domain.eHealth.message.MsgHR_CheckInterval2HR_Unsuitable;

public class HR_NoRelaxedWarning extends MWAgent {
	private static final long serialVersionUID = 8505462503088901786L;

	public boolean log = false;
	private MessageDoctor messageDr;

	public HR_NoRelaxedWarning() {
	}

	protected void setupContend() {
		/** Definir el agente **/
		final String[] sourceComponentIDs = { "compon107" };
		...
		Behaviour running = new CyclicBehaviour(this) {
			...
			public void action() {
				try {
					...
					messageDr.mandarMensajeMedico(receivedData_1.getdato_1(), receivedData_1.getdato_2(), receivedData_1.getdato_3());
