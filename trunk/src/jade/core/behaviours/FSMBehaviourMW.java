 ...
 *          6029 $
 */
public class FSMBehaviourMW extends SerialBehaviour {

	private static final long serialVersionUID = -3993502186887179505L;
	...
	// These variables are used to force a transition on a given state at
	// runtime
	...
	private Logger myLogger = Logger
			.getMyLogger(FSMBehaviourMW.class.getName());
	 ...
	 * 
	 * @param state
	 *            The <code>Behaviour</code> representing the state
	 * @param name
	 *            The name identifying the state.
	 */
	 ...
	 * 
	 * @param s1
	 *            The name of the state this transition starts from
	 * @param s2
	 *            The name of the state this transition leads to
	 * @param event
	 *            The termination event that fires this transition as returned
	 ...
	 *            <code>Behaviour</code> representing state s1.
	 * @param toBeReset
	 *            An array of strings including the names of the states to be
	 *            reset.
	 * @see jade.core.behaviours.Behaviour#onEnd()
	 */
	public void registerTransition(String s1, String s2, int event,
	 ...
	 * @param event
	 *            The termination event that identifies the transition to be
	 *            removed
	 */
	...
	/**
	 * Check if the requested transition exists and is allowed.
	 * 
	 * @param event number of the transition.
	 * @return <code>true</code> if the transition is possible.
	 *         <code>false</code> otherwise.
	 */
	public boolean hasCurrentTransition(int event) {
		return (theTransitionTable.getTransition(currentName, event) != null);
	}
	 ...
	 *         <code>name</code>, or <code>null</code> if no such behaviour
	 *         exists.
	 */
	 ...
	 *         state, or <code>null</code> if the given behaviour is not a child
	 *         of this FSM behaviour.
	 */
	...
	protected void scheduleFirst() {
		if (transitionForced) {
			...
			// TODO modificacion de Rafa para enviar el estado inicial
			try {
				((MWAgent) myAgent).setState(myAgent.getLocalName(),
						currentName);
			} catch (Exception e) {
				e.printStackTrace();
			}
	 ...
	 * 
	 * @param currentDone
	 *            a flag indicating whether the just executed child has
	 *            completed or not.
	 * @param currentResult
	 *            the termination value (as returned by <code>onEnd()</code>) of
	 *            the just executed child in the case this child has completed
	 *            (otherwise this parameter is meaningless)
		...
		if (currentDone) {
			try {
				...
				if (transitionForced) {
					...
					// Normal case: use the TransitionTable to select the next
					// state
					Transition t = theTransitionTable.getTransition(
							currentName, currentResult);
					resetStates(t.toBeReset);
					currentName = t.dest;
				}
				current = getState(currentName);
				// TODO modificacion de Rafa para enviar el estado cada cambio
				// del FSM
				try {
					((MWAgent) myAgent).setState(myAgent.getLocalName(),
							currentName);
			...
			// " is Executing state "+currentName);
		}
	}

	protected void handleInconsistentFSM(String current, int event) {
		throw new RuntimeException("Inconsistent FSM. State: " + current
				+ " event: " + event);
	}
	...
	protected boolean checkTermination(boolean currentDone, int currentResult) {
		...
		if (myLogger.isLoggable(Logger.FINE)) {
			myLogger.log(Logger.FINE, "FSM-Behaviour " + getBehaviourName()
					+ ": checkTermination() returning " + ret);
		}
		// #J2ME_EXCLUDE_END
		return ret;
	}
	 ...
	 * 
	 * @param next
	 *            The name of the state to jump to at the next FSM cheduling
	 *            quantum. If the FSM has no state with the given name, this
	 *            method does nothing.
	 */
	...
	class TransitionTable implements Serializable {
		private Hashtable<String, TransitionsFromState> transitions = new Hashtable<String, TransitionsFromState>();
		...
		void addTransition(Transition t) {
			...
			TransitionsFromState tfs = (TransitionsFromState) transitions
					.get(key1);
	...
	static class Transition implements Serializable {
		...
		public Transition(FSMBehaviourMW f, String s, String d, int t,
				String[] rs) {
			fsm = f;
			src = s;
			dest = d;
		...
		public Transition(FSMBehaviourMW f, String s, String d, String[] rs) {
			fsm = f;
			src = s;
			dest = d;
			trigger = 0;
		...
		public FSMBehaviourMW getFSM() {
			return fsm;
		}

		public void setFSM(FSMBehaviourMW f) {
			fsm = f;
		}
		...
		public String toString() {
			...
			sb.append("(TRANSITION trigger=").append(trigger)
					.append(", source=").append(src).append(", dest=")
					.append(dest).append(")");
			return sb.toString();
		}
		// #MIDP_EXCLUDE_END
