				...
				if (ch != null) {
					...
					try {
						if (deliveryTimeThreshold > 0 && deliveryTime > deliveryTimeThreshold) {
							myLogger.log(Logger.WARNING, "Deliverer Thread "+name+ " - Delivery_Time over threshold ("+deliveryTime+"). Receiver = "+receiverID.getLocalName()+", message size = "+msg.length());
							//ch.notifyFailureToSender(msg, receiverID, new InternalError(ACLMessage.AMS_FAILURE_UNEXPECTED_ERROR));
						}
					}
					catch (Exception e) {
