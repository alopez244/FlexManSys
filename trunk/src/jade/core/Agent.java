...
//#APIDOC_EXCLUDE_END
{
	...
	static final Logger LOGGER = LogManager.getLogger(Agent.class.getName()) ;
  static final Level MESSAGE = Level.forName("MESSAGE", 700);
	...
	public final void send(ACLMessage msg) {
	  if ((msg.getOntology()!=null) && (!msg.getOntology().equals("FIPA-Agent-Management"))) {
	    
	    //LOGGER.log(MESSAGE, msg);//+ ">"  + " ("+msg.getPerformative()+"): " +msg.getContent());
	    
	    StringBuilder receivers = new StringBuilder();
	    for (Iterator i = msg.getAllReceiver(); i.hasNext();)
	      receivers.append(((AID)i.next()).getLocalName()).append(",");
	    if (receivers.length()>0) receivers.deleteCharAt(receivers.length()-1);
	    LOGGER.log(MESSAGE, performativeDesc(msg.getPerformative()) + " > " + receivers + " \"" + msg.getContent() + "\"");
	...
	public String performativeDesc(int code){
	  if (code==ACLMessage.INFORM) return "INFORM";
	  else if (code==ACLMessage.REQUEST) return "REQUEST";
	  return ""+code;
	}
