...
//
CODK_CpuReadData::CODK_CpuReadData(const ODK_VARIANT& classicData)  // Pointer to a classic data structure
    : m_nBytes(0),
      m_data(0)
{
                                    ...
                                    ODK_CHAR*   string) const // Buffer to receive the string characters
{
    ...
    ODK_UINT8  chars2Read = 0; // Maximum chars to read
    ...
    if ((byteOffset < 0)  ||  ((byteOffset + static_cast<long>(sizeof(ODK_CHAR) * (maxLen + 2))) > m_nBytes))
    {
        return false;
    }
    ...
    {
        chars2Read = static_cast<ODK_UINT8>(readMax - 1);
    }

    // Read the value
    memcpy(string, &(m_data[byteOffset + (sizeof(ODK_CHAR) * 2)]), sizeof(ODK_CHAR) * chars2Read);
    string[chars2Read] = '\0';  // terminate the string
...
//
bool CODK_CpuReadData::ReadS7WSTRING_LEN(const long  byteOffset,    // Offset to begin reading
                                         ODK_UINT16  &maxLen,       // Maximum length of the string
                                         ODK_UINT16  &curLen) const // Current length of the string
{
    if (ReadUINT16(byteOffset, maxLen)  &&  ReadUINT16(byteOffset + sizeof(ODK_WCHAR), curLen))
    {
        return true;
    }
...
//                 Truncate destination string, when source string is too long.
//
bool CODK_CpuReadData::ReadS7WSTRING(const long  byteOffset,   // Offset to begin reading
...
{
    ODK_UINT16  curLen  = 0; // Current length of the string
    ODK_UINT16  maxLen  = 0; // Maximum length of the string
    ODK_UINT16  chars2Read = 0; // Maximum chars to read
    ...
    // get the current string length
    if (!ReadS7WSTRING_LEN(byteOffset, maxLen, curLen))
    {
        return false;
    }

    // verify offset not out of bounds
    if ((byteOffset < 0) || ((byteOffset + static_cast<long>(sizeof(ODK_WCHAR) * (maxLen + 2))) > m_nBytes))
    {
        return false;
    }
    ...
    {
        chars2Read = static_cast<ODK_UINT16>(readMax - 1);
    }

    // Read the value
    long offset = byteOffset + (sizeof(ODK_WCHAR) * 2);
    ODK_UINT16 word;
    for (ODK_UINT16 i = 0; i < chars2Read; i++, offset += sizeof(ODK_WCHAR))
    {
        word = reinterpret_cast<ODK_UINT16&>(m_data[offset]);
        string[i] = (((word & 0x00FFU) << 8) |
                     ((word & 0xFF00U) >> 8));
    }
    string[chars2Read] = '\0';  // terminate the string
