    ...
    private static final long serialVersionUID = 6711046229173067015L;
        ...
        public void loadPredefined(String cmd) {
          ...
          if (cmd.equals("loadnum")) { // n componentes
            ...
            //Adaptability: sobre las 5 raspberries lanzar evento de reconfiguración que lanza aplicación con 2, 4, 8, 16, 32 componentes. Medir tiempo hasta arranque de la aplicación. 
            ...
            for (int j=1; j<=1;j++) {
              ...
              //Aplicación
              sendCommand(mwm,"reg application ID=applic"+(100+componentes)+" name=Application"+(100+componentes)+" parent="+scenID+" desc=Aplicaciï¿½n de ejemplo 1 activation=periodic period=1000 deadline=1000");
              //Generador
            ...
            String action1ID = sendCommand(mwm,"reg action name=Accion1 parent="+event1ID+" desc=Arrancar aplicación action=start compon101").getContent();
            String action2ID = sendCommand(mwm,"reg action name=Accion2 parent="+event1ID+" desc=Parar aplicación action=start compon102 order=2").getContent();
            String even1Scn1Act1ID = sendCommand(mwm,"reg action name=Accion3 parent="+event1ID+" desc=Para aplicación action=start compon103 order=3").getContent();
            ...
            String actionSPID = sendCommand(mwm,"reg action name=actSetEvent parent=eventSetGenPeriod desc=Cambia temporización action=\"localcmd (getins compon101) cmd=set period 1000\"").getContent();
            ...
            sendCommand(mwm,"reg action ID=actionStopRPC name=StopapplicPRC parent=eventStopapplicPRC desc=destruye la aplicación PRC action=\"localcmd applicPatientRelaxedChecking cmd=setstate stop\"");
            ...
            //Tipo de acción asociada al evento = ModifyQoS
            //Se cambia el periodo de la aplicación, que en realidad es el periodo del componente HR_Acquisition que es el único periódico.
            ...
            sendCommand(mwm,"reg action ID=actionStartBPM name=StartApplicBPM parent=eventStartApplicBPM desc=inicia la aplicación BPM action=\"start applicPatientBPM\"");
            ...
            String app1ID = sendCommand(mwm,"reg application name=sputnik1 parent="+scenID+" desc=Aplicaciï¿½n de ejemplo 1 activation=periodic period=1000").getContent();
            ...
            //        
            //        cmd = "reg action name=Accion1 parent="+event1ID+" desc=Arrancar aplicación "
            //            + "action=start " + com1ID ;
            //        String action1ID = sendCommand(mwm, cmd).getContent();
            //        LOGGER.info(cmd+" : "+action1ID);
            //        
            //        cmd = "reg action name=Accion2 parent="+event1ID+" desc=Parar aplicación "
            //            + "action=start " + com2ID + " order=2";
            //        String action2ID = sendCommand(mwm, cmd).getContent();
            //        LOGGER.info(cmd+" : "+action2ID);
            //        
            //        cmd = "reg action name=Accion3 parent="+event1ID+" desc=Para aplicación "
            ...
            cmd = "reg application name=Application1 parent="+scenID+" desc=Aplicaciï¿½n de ejemplo 1"
                + " activation=periodic period=1000 deadline=1000";
